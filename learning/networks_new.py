...
class GNN(linen.Module):
  """Simple GNN module with adjacency mask."""
  ...
  def __call__(self, data: jnp.ndarray):
    # data shape: (batch, num_nodes * node_dim)
    orig_shape = data.shape

    x = data.reshape(-1, orig_shape[-1])   # (N,32)
    x = x.reshape(-1, self.num_nodes, self.node_dim)  # (N,16,2)

    # simple network with direcrt neighbors
    A = jnp.eye(self.num_nodes)
    for i in range(self.num_nodes - 1):
      A = A.at[i, i+1].set(1.0)
      A = A.at[i+1, i].set(1.0)
    A = A / jnp.sum(A, axis=-1, keepdims=True)

    for i, hidden_size in enumerate(self.layer_sizes):
      x = jnp.einsum("ij,bjk->bik", A, x)
      x = linen.Dense(hidden_size, name=f'gnn_{i}')(x)
